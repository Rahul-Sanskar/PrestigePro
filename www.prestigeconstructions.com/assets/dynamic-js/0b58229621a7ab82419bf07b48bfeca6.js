const urlParams = new URLSearchParams(window.location.search);
  ...
  //var lastPart = "bangalore"
  // var lastPart = "hyderabad"
  // var lastPart = "cochin"
...
if (checkNUll(lastPart) !== "") {
    ...
    // Check if the city name includes "ncr" or "delhi-ncr" and set it to "Delhi-NCR" statically
    if (lastPart.toLowerCase() === "ncr" || lastPart.toLowerCase() === "delhi-ncr") {
        capitalizedCity = "Delhi-NCR";
    } else {
        // Capitalize other city names normally
        capitalizedCity = lastPart.charAt(0).toUpperCase() + lastPart.slice(1).toLowerCase();
    }

    $(".bindcitytitle").html("Residential Projects in " + capitalizedCity);
}

// Dynamic Property Types Data Condition-wise :: start
let propertyTypesData = {
    "bangalore": [
        { name: "Townships", url: "https://www.prestigeconstructions.com/residential-projects/bangalore/township-in-bangalore" },
         { name: "Apartments", url: "https://www.prestigeconstructions.com/residential-projects/bangalore/apartments-in-bangalore" },
        //{ name: "Apartments", url: "https://www.prestigeconstructions.com/properties-in-whitefield-test/apartments-in-bangalore" },
        { name: "Villa", url: "https://www.prestigeconstructions.com/residential-projects/bangalore/villa-in-bangalore" },
        { name: "Plotted Developments", url: "https://www.prestigeconstructions.com/residential-projects/bangalore/plots-in-bangalore" }
    ],
    "hyderabad": [
        { name: "Townships", url: "https://www.prestigeconstructions.com/residential-projects/hyderabad/township-in-hyderabad" },
        { name: "Apartments", url: "https://www.prestigeconstructions.com/residential-projects/hyderabad/apartments-in-hyderabad" },
        { name: "Villa", url: "https://www.prestigeconstructions.com/residential-projects/hyderabad/villa-in-hyderabad" }
    ],
    "mumbai": [
        { name: "Townships", url: "https://www.prestigeconstructions.com/residential-projects/mumbai/township-in-mumbai" },
        { name: "Apartments", url: "https://www.prestigeconstructions.com/residential-projects/mumbai/apartments-in-mumbai" },
        // { name: "Villa", url: "https://www.prestigeconstructions.com/residential-projects/mumbai/villa-in-mumbai" },
        { name: "Luxury Apartments", url: "https://www.prestigeconstructions.com/residential-projects/mumbai/luxury-apartments-in-mumbai" }
    ],
    "chennai": [
        { name: "Apartments", url: "https://www.prestigeconstructions.com/residential-projects/chennai/apartments-in-chennai" },
        { name: "Villa", url: "https://www.prestigeconstructions.com/residential-projects/chennai/villa-in-chennai" }
    ]
};

// Populate the #PropertyTypes dropdown based on the city
let currentCityKey = (lastPart || "").toLowerCase();
let typesToShow = propertyTypesData[currentCityKey];

let $PropertyTypes = $("#PropertyTypes");
let $dropdownContainer = $PropertyTypes.closest("li");

if (typesToShow && typesToShow.length > 0) {
    // City found in our data, show dropdown and populate
    $dropdownContainer.show();
    $PropertyTypes.empty();
    $PropertyTypes.append('<option class="text-capitalize" data-description="" value="">Property Types</option>');
    $.each(typesToShow, function(index, typeData) {
        $PropertyTypes.append('<option class="text-capitalize" value="' + typeData.name + '" data-url="' + typeData.url + '">' + typeData.name + '</option>');
    });
    
    // Reset to default "Property Types" placeholder (Fixes back button issue)
    $PropertyTypes.val("");

    // Redirect on selection change
    $PropertyTypes.off('change.urlRedirect').on('change.urlRedirect', function() {
        let selectedUrl = $(this).find('option:selected').attr('data-url');
        if (selectedUrl) {
            // Reset the dropdown immediately before redirecting. 
            // This ensures if the user clicks the back button, they see "Property Types" selected.
            $(this).val("");
            
            // setTimeout to ensure the browser has time to register the change if needed
            setTimeout(function() {
    ...
    // Handle bfcache (browser back button) where JS might not re-run fully
    ...
    // City not in our data, hide the dropdown completely
    $dropdownContainer.hide();
}
// Dynamic Property Types Data Condition-wise :: end
  ...
  $(document).ready(function() {
    ...
    var citylocationobj = localStorage.getItem("clickedcity");
    if (citylocationobj == "all") {
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if (response.success == true && response.data.length > 0) {
          ...
          // var delhiCity = `<li>
          //         <a href="https://www.prestigeconstructions.com/residential-projects/delhi-ncr" data-city="delhi-ncr">
      ...
      complete: function() {
    //  <a href="https://www.prestigeconstructions.com/residential-projects"> 
        // <a href="JAVASCRIPT:;" class="allprojectload" data-city="all">
        var projectcity = `<li>
                                      
                                        <a href="https://www.prestigeconstructions.com/residential-projects" class="allprojectload" data-city="all">
...
$(document).on("click", ".allprojectload", function(e) {
  ...
  function projectlist(citycode = "", search = "", type = "", minprice = "", maxprice = "", configuration = "", construction = "", directiontype) {
    ...
    $.ajax({
      ...
      complete: function() {
        ...
        localStorage.setItem("clickedcity", "");
        get_location_rounded_pill(lastPart, "Residential", "dataactive direction_click", directiontype);
        if ($("#is_active_filter").val() == "yes") {
          ...
          if (item._id == construction) {
            if (checkNUll(item.ProjectStatus) != "") {
              var row = `<div class="custom-checkbox-btn">
                                        <input class="type" type="checkbox" id="${item.ProjectStatus}" data-status="${item.ProjectStatus}" name="status" value="${item._id}" checked>
            ...
            if (checkNUll(item.ProjectStatus) != "") {
              var row = `<div class="custom-checkbox-btn">
                                        <input class="type" type="checkbox" id="${item.ProjectStatus}" data-status="${item.ProjectStatus}" name="status" value="${item._id}">
                                        <label for="${item.ProjectStatus}">${item.ProjectStatus}</label>
                                    </div>`;
              $(".bins_status").append(row);
            }
          ...
          if (item._id == dbedrooms) {
            if (checkNUll(item.typename) != "") {
              if (item._id !== "6596793831bf9d0018f4d5ef") {
                var row = `<div class="custom-checkbox-btn">
                                        <input type="checkbox" id="${item.typename}" name="beds" value="${item._id}" data-value="${item.typename}"checked>
            ...
            if (checkNUll(item.typename) != "") {
              if (item._id !== "6596793831bf9d0018f4d5ef") {
                var row = `<div class="custom-checkbox-btn">
                                        <input type="checkbox" id="${item.typename}" name="beds" value="${item._id}" data-value="${item.typename}">
                                        <label for="${item.typename}">${item.typename}</label>
                                    </div>`;
                $(".bind_search_beds").append(row);
              }
  ...
  $("#countrycode").change(function() {
    ...
    function toggleWhatsAppVisibility() {
            ...
            if (selectedCountryCode === "IN") {
                $(".enquire_is_whatsapp").addClass("is-hidden"); // Hide WhatsApp section
                $(".resend_otp_btn_request_call_back").addClass("is-hidden"); // Hide Resend OTP button
            $("#timer_display").hide(); // Hide the timer
            } else {
                $(".enquire_is_whatsapp").removeClass("is-hidden"); // Show WhatsApp section
      ...
      // API NOT MEARGED IN DEV
       if ($("#whatsapp").is(":checked")) {
                ...
                function generateWhatsAppLink(waMobileNo, waCountryCode, wa_age_consent, wa_marketing_consent) {
                    ...
                    formdata["Page_Name"]          = $(".bindcitytitle").text() + " - Enquiry Now";
      ...
      success: function(result) {
        ...
        $.each(result.data, function(i, item) {
          if (item.ProjectName == projectname) {
            var obj = '<option class="text-capitalize"   data-description="' + item.ProjectName + '" value="' + item.ProjectID + '" selected>' + item.ProjectName + '</option>';
            $("#booking_project").parent().addClass("focused");
          } else {
            var obj = '<option class="text-capitalize"   data-description="' + item.ProjectName + '" value="' + item.ProjectID + '">' + item.ProjectName + '</option>';
          }
    ...
    submitHandler: function(form) {
      ...
      if ($("#whatsapp_book").is(":checked")) {
                ...
                function generateWhatsAppLink(waMobileNo, waCountryCode, wa_age_consent, wa_marketing_consent) {
                    ...
                    formdata["Mobile_No"]          = waMobileNo;
                    formdata["Mobile_CountryCode"] = waCountryCode;
                    formdata["Page_Name"]          = $(".bindcitytitle").text() + " - Book a Site Visit";
                    formdata["Plateform_Name"]     = "web";
                    formdata["RequestFrom"]        = "Website";
                    formdata["Session_ID"]         = socketId;
                    formdata["Type"]               = "Enquiry Now";
...
//Moengage_event start:Residential_city_page_direction
$(document)
  .off("click", ".bind_location_basedon_city a")
  .on("click", ".bind_location_basedon_city a", function (e) {
      var href = $(this).attr("href");
      
      // Prevent default only if href is void or #
      if (href === "javascript:void(0);" || href === "#" || href === "") {
          e.preventDefault();
      }

      var direction = $(this).data("directiontype") || "";
      var text = $(this).text().trim();
      var pageUrl = window.location.href; // current page URL
      var isActive = $(this).hasClass("active");

      // If direction is empty, use text instead
      direction = direction || text;

      // MoEngage tracking
      if (typeof Moengage !== "undefined" && Moengage.track_event) {
          Moengage.track_event("Residential_city_page_direction", {
              direction: direction,
              text: text,
              page_url: pageUrl,
              active: isActive,
              ip_address: window.userIP || "",
              event_name: "Residential_city_page_direction",
              event_type: ""
          });
      }
  });
  //Moengage_event end:Residential_city_page_direction

  $(document).ready(function() {
      // Toggle active class on Property Types dropdown
      $('#PropertyTypes').on('select2:open', function() {
